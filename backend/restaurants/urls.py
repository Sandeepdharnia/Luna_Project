...
from restaurants.views import FillView, ListCreateRestaurant, GetRestaurantsFromSpecificUser, \
    RetrieveUpdateDeleteSpecificRestaurant, ListRestaurantByGivenCategory

urlpatterns = [
    path('', ListCreateRestaurant.as_view()),   # list all restaurants
    path('new/', FillView.as_view()),   # create a new restaurant
    path('category/<int:category_id>/', ListRestaurantByGivenCategory.as_view()),    # get all restaurants from given category (by id)
